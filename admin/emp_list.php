				...
				<div class="row">
					<form method="post" class="form-inline">
						<div class="form-group" style="margin-left:1.5%;">
							<input class="btn" id="emp" type="submit" name="emp" value="EMPLOYEE">
							<input class="btn" type="submit" name="dept" value="DEPARTMENT">
							<input class="btn" type="submit" name="proj" value="PROJECTS">
						</div>
					...
					<?php
						...
						if($_POST['emp']) {
							$emp = $_POST['emp'];
							$emp_show = "SELECT * FROM emp_details";
							$emp_count = "SELECT COUNT(emp_id) AS num FROM emp_details";
							$emp_res = mysqli_query($connection, $emp_show);
							$emp_count_res = mysqli_query($connection, $emp_count);
							$list = mysqli_fetch_assoc($emp_count_res);
							if(mysqli_num_rows($emp_res) > 0) {
								echo "<div class='col-md-12' style='padding: 20px;'><div class='card'>";
								echo "<div class='header'>
										<h4 class='title'><b>Employee List</b> | <small>no. of employees:</small> ".$list['num']."
								...
								echo "<thead><tr style='border-top: 1px solid lightgrey;'>
										<th style='padding-left: 17px;'><h5><b>Employee ID</b></h5></th>
										<th><h5><b>First Name</b></h5></th>
										<th><h5><b>Designation</b></h5></th>
										<th><h5><b>Department ID</b></h5></th>										
										<th><h5><b>Date of Join</b></h5></th>										
									</tr></thead><tbody id='myTable'>";
								while($row = mysqli_fetch_assoc($emp_res)){
									echo "<tr>
											<td><a href='emp_profile.php' style='color:indigo;cursor:pointer;'>".$row['emp_id']."</a></td>
											<td><a style='color:indigo;cursor:pointer;'>".$row['f_name']."</a></td>
											<td>".$row['desig']."</td>
											<td>".$row['dept_id']."</td>
											<td>".$row['doj']."</td></tr>";									
								}
						...
						elseif($_POST['dept']) {
							...
							if(mysqli_num_rows($dept_res) > 0) {
								...
								echo "<thead><tr style='border-top: 1px solid lightgrey;'>
										<th style='padding-left: 14px;'><h5><b>Department ID</b></h5></th>
										<th><h5><b>Department Name</b></h5></th>
										<th><h5><b>Department Head</b></h5></th>										
										<th><h5><b>Building</b></h5></th>
									</tr></thead><tbody id='myTable'>";
								while($row = mysqli_fetch_assoc($dept_res)){
									echo "<tr>
											<td><a style='color:indigo;cursor:pointer;'>".$row['dept_id']."</a></td>
											<td><a style='color:indigo;cursor:pointer;'>".$row['dept_name']."</a></td>
											<td>".$row['dept_head']."</td>											
											<td>".$row['building']."</td></tr>";								
								}
								echo "</tbody></table></div>";
								echo "</div></div>";
								echo "<script>
						...
						elseif($_POST['proj']) {
							$proj = $_POST['proj'];
							$proj_show = "SELECT * FROM projects";
							$proj_count = "SELECT COUNT(proj_id) AS num FROM projects";
							$proj_count_res = mysqli_query($connection, $proj_count);
							$list = mysqli_fetch_assoc($proj_count_res);
							$proj_res = mysqli_query($connection, $proj_show);										
							if(mysqli_num_rows($proj_res) > 0) {
								echo "<div class='col-md-12' style='padding: 20px;'><div class='card'>";
								echo "<div class='header'>
										<h4 class='title'><b>Projects List</b> | <small>no. of projects:</small> ".$list['num']."
											<div class='col-xs-3 pull-right'>
												<input class='form-control' id='myInput' type='text' class='fa fa-search' placeholder=' Search..' style='border:1px solid #000;'>
											</div>
										</h4>
								...
								echo "<div class='content table-responsive table-full-width'><table class='table table-hover table-striped'>";
								echo "<thead><tr style='border-top: 1px solid lightgrey;'>
										<th style='padding-left: 17px;'><h5><b>Project ID</b></h5></th>
										<th><h5><b>Project Name</b></h5></th>
										<th><h5><b>Project Head</b></h5></th>
										<th><h5><b>Deadline</b></h5></th>
										<th><h5><b>Status</b></h5></th>
									</tr></thead><tbody id='myTable'>";
								while($row = mysqli_fetch_assoc($proj_res)){
									echo "<tr>
											<td><a style='color:indigo;cursor:pointer;'>".$row['proj_id']."</a></td>
											<td><a style='color:indigo;cursor:pointer;'>".$row['proj_name']."</a></td>
											<td>".$row['proj_head']."</td>											
											<td>".$row['deadline']."</td>
											<td>".$row['status']."</td></tr>";								
								}
